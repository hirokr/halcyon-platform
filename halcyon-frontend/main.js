...
class HalcyonApp {
  ...
  initializeMap() {
      ...
      layer.bindTooltip(
        `
                <div class="popup-content">
                    <h4>${ward.name}</h4>
                    <div class="popup-urvi">URVI: ${this.calculateURVI(
                      ward
                    )}</div>
                    <div class="popup-risk-level ${this.getRiskLevelClass(
                      this.calculateURVI(ward)
                    )}">${this.getRiskLevel(this.calculateURVI(ward))}</div>
                </div>
            `,
        {
          permanent: false,
    ...
    console.log(
      ...
      Object.keys(this.wardLayers).length,
      "wards"
    );
  }
  ...
  setupEventListeners() {
    ...
    document
      ...
      .addEventListener("click", () => this.simulateIntervention());

    // Export button
    document
      .getElementById("exportBtn")
      .addEventListener("click", () => this.exportActionBrief());
  }
  ...
  normalizeWeights() {
    const total = Object.values(this.currentWeights).reduce(
      (sum, val) => sum + val,
      0
    );
    ...
    if (total !== 100) {
      ...
      Object.keys(this.currentWeights).forEach((key) => {
        this.currentWeights[key] = Math.round(
          this.currentWeights[key] * factor
        );
      });
  ...
  showWardAnalysis(ward) {
    ...
    document.getElementById("ward-population").textContent =
    ...
    document.getElementById("ward-risk-level").textContent =
      this.getRiskLevel(urvi);

    // Update infrastructure stats
    document.getElementById("ward-hospitals").textContent =
      ward.infrastructure.hospitals;
    document.getElementById("ward-schools").textContent =
      ward.infrastructure.schools;
    document.getElementById("ward-facilities").textContent =
      ward.infrastructure.critical_facilities;
  ...
  simulateIntervention() {
    const interventionType =
      document.getElementById("interventionSelect").value;
    ...
    if (resultsDiv && impactDisplay) {
      impactDisplay.innerHTML = impacts
        .map(
          (impact) => `
  ...
  updateWardDisplay() {
    ...
    if (riskLevelElement)
      riskLevelElement.textContent = this.getRiskLevel(urvi);
...
- Infrastructure Vulnerability: ${
      ward.risk_factors.infrastructure_vulnerability
    }%
...
Data Sources: NASA Earth Science Data, OpenStreetMap, Dhaka City Corporation
    ...
    a.href = url;
    a.download = `halcyon-action-brief-${ward.name
      .toLowerCase()
      .replace(/\s+/g, "-")}.txt`;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  try {
    ...
    console.log("Halcyon App initialized successfully");
  } catch (error) {
    console.error("Error initializing Halcyon App:", error);
    // Display error message to user
    const errorDiv = document.createElement("div");
    errorDiv.style.cssText =
      "position: fixed; top: 10px; right: 10px; background: #f44336; color: white; padding: 10px; border-radius: 4px; z-index: 9999;";
    errorDiv.textContent = "JavaScript Error: Check console for details";
